...
(defconst org-element--citation-prefix-re
  (rx (seq "[cite"
	   (opt (group (one-or-more "/" (one-or-more alnum)))) ;style
	   ":"
	   (zero-or-more (any "\t\n "))))
  ...
  (when (looking-at org-element--citation-prefix-re)
    (let* ((begin (point))
	   (style (and (match-end 1)
		       ;; Remove initial "/".
		       (substring (match-string-no-properties 1) 1)))
		...
		(save-excursion
		  (org-element-put-property
		   ...
		   (org-element-parse-secondary-string
		    (buffer-substring start (point)) restriction cite))))
	      (forward-char)
		...
		(when (< (1+ (point)) end)
		  (save-excursion
		    ...
		    (org-element-put-property
		     cite :suffix
		     (org-element-parse-secondary-string
		      (buffer-substring (point) end) restriction cite))))
  ...
  (save-excursion
    (let ((begin (point)))
      ...
      (let* ((key (match-string-no-properties 1))
	     ...
	     (key-end (match-end 0))
	     (restriction (org-element-restriction 'citation-reference))
	     (end (let ((next (search-forward ";" nil t)))
		    (if next (1- next) (point-max))))
	     (reference
	...
	(when (< begin key-start)
	  (org-element-put-property
	   reference :prefix
	   (org-element-parse-secondary-string
	    (buffer-substring begin key-start) restriction reference)))
	(when (< key-end end)
	  (org-element-put-property
	   reference :suffix
	   (org-element-parse-secondary-string
	    (buffer-substring key-end end) restriction reference)))
	reference))))
